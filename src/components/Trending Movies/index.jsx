...
function Trending({ text }) {
  const { QueriesObject, favourites, setFavourites } =
    useContext(GlobalContext);
  const data = QueriesObject[text][0].data.results;
  const genre = QueriesObject[text][1].data.genres;
  const navigate = useNavigate()
  if (QueriesObject[text].isLoading) return <Variants />;

  useEffect(() => {
    localStorage.setItem("favourites", JSON.stringify([...favourites]));
  }, [favourites]);
  return (
    <section className="max-w-[1300px]  h-min(400px,100vh) px-[6px] py-[32px] lg:ml-[280px] ml-[10px]" >
      <div className="text-white text-[20px] pb-4 font-[500]">{text}</div>
      <div className="flex items-start justify-start gap-x-[23px] overflow-x-scroll overflow-y-hidden ">
        {data.map((trend, index) => {
          ...
          return (
            <div
              key={index}
              className="w-[255px] h-[301px] flex flex-col items-start justify-start relative rounded-sm"
              data-id={trend.id} onClick={()=>{
                navigate(`/Home/${text}/${trend.id}`)
              }}
            >
              ...
              <button
                ...
                onClick={() => {
                  // console.log('Initial','Like:',like,'isLiked:',isLiked)
                  setFavourites((prev) => {
